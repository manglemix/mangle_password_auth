...
use mangle_db_enums::GatewayRequestHeader;
...
pub struct AuthState {
	...
	used_challenges: RwLock<Vec<String>>,
	...
	user_privilege_map: RwLock<HashMap<ArcString, Vec<UserPrivileges>>>
}
...
impl AuthState {
			...
			if let Some(privs) = user_table.get("privileges") {
				...
				if let Value::Array(privs) = privs {
					for privilege in privs {
						privileges.push(unwrap_result_or_default_error!(
	...
	pub async fn is_valid_key(&self, username: &String, challenge: String, signature: Signature) -> bool {
		...
		match key.verify_strict(challenge.as_bytes(), &signature) {
			Ok(()) => {
				self.used_challenges.write().await.push(challenge);
				true
			}
			Err(_) => false
		}
	...
	pub async fn get_user_privileges(&self, username: &String) -> Vec<UserPrivileges> {
